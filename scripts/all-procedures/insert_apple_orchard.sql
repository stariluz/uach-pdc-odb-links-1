...
BEGIN
    ...
    IF LOWER(p_server) = 'chihuahua' THEN
        INSERT INTO apple_chihuahua (orchard_name, location, server)
        VALUES (p_orchard_name, p_location, LOWER(p_server));

    ELSIF LOWER(p_server) = 'cuauhtemoc' THEN
        INSERT INTO apple_cuauhtemoc (orchard_name, location, server)
        VALUES (p_orchard_name, p_location, LOWER(p_server));

    ELSIF LOWER(p_server) = 'juarez' THEN
        INSERT INTO apple_juarez (orchard_name, location, server)
        VALUES (p_orchard_name, p_location, LOWER(p_server));
