...
BEGIN
    ...
    FROM (
        ...
        UNION
        SELECT server
        FROM apple_orchards@juarez_chihuahua ao2
        JOIN apple_orchard_sales@juarez_chihuahua aos2 ON ao2.orchard_name = aos2.orchard_name
        WHERE aos2.sale_id = p_sale_id
        UNION
        SELECT server
        FROM apple_orchards@juarez_cuauhtemoc ao3
        JOIN apple_orchard_sales@juarez_cuauhtemoc aos3 ON ao3.orchard_name = aos3.orchard_name
